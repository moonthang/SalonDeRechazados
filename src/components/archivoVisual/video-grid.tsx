...
export default function PostGrid({ selectedTag, searchQuery }: PostGridProps) {
    ...
    const fetcher = useCallback(async (tag: string, search: string, startAfterDoc: QueryDocumentSnapshot<DocumentData> | null) => {
        ...
        try {
            ...
            // Nota: El filtrado por título en Firestore es sensible a mayúsculas y limitado a prefijos.
            // Para una búsqueda más flexible (contiene texto), se implementa un filtro manual básico si hay búsqueda.
            // Opcionalmente, se podría usar prefix matching: where('title', '>=', search).where('title', '<=', search + '\uf8ff')
            
            const q = query(episodesRef, ...constraints, orderBy('createdAt', 'desc'));
            const querySnapshot = await getDocs(q);
            
            let allResults = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as AfterglowEpisode));
            
            if (search) {
                allResults = allResults.filter(ep => 
                    ep.title.toLowerCase().includes(search.toLowerCase())
                );
            }

            // Paginación manual simple para el prototipo cuando hay búsqueda/filtros complejos
            const startIndex = startAfterDoc ? allResults.findIndex(ep => ep.id === startAfterDoc.id) + 1 : 0;
            const newEpisodes = allResults.slice(startIndex, startIndex + PAGE_SIZE);
            
            const newHasMore = (startIndex + PAGE_SIZE) < allResults.length;
            const newLastDoc = querySnapshot.docs.find(d => d.id === newEpisodes[newEpisodes.length - 1]?.id) || null;
    ...
    useEffect(() => {
        ...
        fetcher(selectedTag, searchQuery, null).then(({ newEpisodes, newLastDoc, newHasMore }) => {
    ...
    }, [selectedTag, searchQuery, fetcher]);

    const loadMoreEpisodes = useCallback(() => {
        if (loading || loadingMore || !hasMore || !lastDoc) return;

        setLoadingMore(true);
        fetcher(selectedTag, searchQuery, lastDoc).then(({ newEpisodes, newLastDoc, newHasMore }) => {
    ...
    }, [loading, loadingMore, hasMore, lastDoc, selectedTag, searchQuery, fetcher]);
    ...
    if (episodes.length === 0) {
        return (
            <div className="text-center py-16">
              <p className="text-lg text-muted-foreground">No se encontraron publicaciones que coincidan con los filtros.</p>
            </div>
        );
    }
